...
#include "moon/physics/box2d/ChainShape.h"
...
void ChainShape::setNextVertex(float x, float y)
{
	if (loop)
	{
		throw moon::Exception("Physics error: Can't call setNextVertex on a loop ChainShape");
		return;
	}
	b2Vec2 v(x, y);
...
void ChainShape::setPreviousVertex(float x, float y)
{
	if (loop)
	{
		throw moon::Exception("Physics error: Can't call setPreviousVertex on a loop ChainShape");
		return;
	}
	b2Vec2 v(x, y);
	b2ChainShape *c = (b2ChainShape *)shape;
...
b2Vec2 ChainShape::getPoint(int index) const
{
	...
	if (index < 0 || index >= c->m_count)
		throw moon::Exception("Physics error: index out of bounds");
