...
#include "moon/physics/Shape.h"
#include "moon/physics/box2d/Fixture.h"
...
int Body::getFixtureList(lua_State *L) const
{
	...
	do
	{
		...
		if (!fixture)
			throw moon::Exception("A fixture has escaped Memoizer!");
...
int Body::getJointList(lua_State *L) const
{
	...
	do
	{
		...
		if (!joint)
			throw moon::Exception("A joint has escaped Memoizer!");
